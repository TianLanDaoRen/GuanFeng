...
- **只发聚合统计**：周期天数、有效天数、气压大变化日与日期、打卡次数与标签/强度分布、每日气压极值。
  **不发**逐条打卡时间戳、备注原文、体征逐点数据、任何设备或用户标识——
  `AiDigestTest` 里有一条测试专门断言这一点。
...
| MVP-10 体感佐证（光照 / 心率 / 腕温） | ✅ 完成，仅光照参与判定（依据见 9.5） |
