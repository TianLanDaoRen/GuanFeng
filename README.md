...
**① 观风**
中间那个**环代表气压变化情况**——指针落在哪一段，就是此刻气压在怎么变：左端「急降」、右端「急升」。环心自上而下：**风雨倾向**（标签）→ **低 / 中 / 高**（大字）→ 环下三格 **急降 / 当前区段 / 急升**
（同一字号同一基线，中间那格随指针走并用该区段色）。最下方一行是判断依据（3 小时净变压）。
...
改用**天气过程状态机**：**3 小时内净降 ≥ 2.0 hPa** 即进入"可能下雨"（2026-09-14 前是"不限时累计 1.5"，
那个参照最高点永不遗忘，一整天慢慢降也算数）；解除仍看**自最低点回升 1.5 hPa**，
另加**12 小时超时**：连续 12 小时既没创新低、也没回升到解除线就自动解除（治"过程挂一整天不消失"）。两个门限同量级是刻意的——雨带间歇期气压本来就上下抖（±0.6，峰峰 1.2），门限不对称会让状态来回翻、提醒跟着刷屏。灰色地带保持原状态，这就是迟滞。

判据本身取自**暴风定律**（**3 小时净降 3 hPa = 雷暴起点**，6 hPa = 强风，10 hPa = 大风；
我们取 3/6 两档作为中度/暴风级），但改成看**净变幅**而不是斜率——先急降后转平这条最典型的降雨曲线，用斜率回归必然被当成"抖动过大"。
...
| 文件 | 内容 |
|---|---|
| [`docs/dev.md`](docs/dev.md) | 开发者文档：算法推导、真机实测结论、阈值出处、未验证清单；天气与定位整条链路在 **§9.12**，功耗优化与三个 bug 的机制在 **§十四**，口径统一（文献锚点、单位、建议映射、过程状态机）在 **§十八** |
...
### 高度解耦与计步（2026-09-14 夜，真机黑盒验收）

观风会把**人在竖直方向的移动**从气压里扣掉，只留天气分量。三条判据：

| 判据 | 阈值 | 依据 |
|---|---|---|
| **快速运动** | `\|rate\| > 4 hPa/分` | 真实天气 ≤0.056 hPa/分（雷暴起点 3 hPa/3h）；电梯实测 5~12 |
| **慢速运动** | **步态** + 3 分钟单向 ≥0.2 hPa | 车库坡实测 0.28 hPa/分、楼梯 0.5；天气 3 分钟最多 0.17 |
| **静止/平走** | 无步态或净变≈0 → **一律不扣** | 真实天气必须原样保留 |

**计步源 2026-09-14 换过**：原先一步一事件的 `STEP_DETECTOR`，一小时健走只记到
**223 步**（运动记录真值 6327，仅 **3.5%**），且与传感器档位无关；
现改用 `STEP_COUNTER`（累计步数）**做差**——真机验证走路 **86 步/分**、静坐 **0 步**。

**验收（同一批黑盒数据）**：电梯上/下行捕获 **83% / 80%**、车库缓坡 **68%**，
而**平走一小时与静置 3.5 小时均为 0 误差**——静止时那 0.34 hPa 的真实天气变化
被完整保留（不会被当成"人在动"扣掉）。

**已知残余**：极端对流（≥0.2 hPa/分）且正在走路时会被误扣；一经停下即恢复如实记录。
