...
2. **喂进桶的必须是天气分量，不是气压计原始读数。** 这是本项目最值得记的一次教训：
   ...
   会被算成一次「气压大变化日」，打卡点也会落在被高度污染的位置上。修法见 9.4。

### 9.2 小时归档：碎片化与保留策略

原始会话 CSV 有两个毛病：**碎片化**（每次应用/服务重启新开一个文件，
实测 11 小时产生 14 个）与**体积**（5 秒采样约 2 MB/天）。
于是"保留多少原始数据"和"能不能看长期历史"被绑死：删文件就没历史，留历史就吃满 ROM。

拆开的办法——**小时归档（`hourly.csv`）与原始文件各司其职**：

| 文件 | 内容 | 保留 |
|---|---|---|
| `guanfeng_*.csv` | 5 秒明细，服务最近 3 小时趋势窗口与高度解耦 | 保留最近 **30 个**（按数量，不按天数） |
| `hourly.csv` | 小时汇总（气压分量 + 心率/腕温/光照），约 1 KB/天 | **永不删除** |

**不要"把旧文件里提取的数据抄进新会话文件"**——那会让同一批样本在分析时被算两遍，
而且很难事后发现。跨文件读取 / 单一归档才是正确做法。
归档按整点去重（保留样本数最多的一行），兜底"整点前后各重启一次"的重复落盘。

实测（本机 pull 下来的 14 个真实文件、1865 行）：**跨文件与文件内均零重复时间戳**。
...
| MVP-10 体感佐证（光照 / 心率 / 腕温） | ✅ 完成，仅光照参与判定（依据见 9.6） |
