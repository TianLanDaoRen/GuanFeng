 ...
 * AI 上报摘要的构造。
 *
 * ## 隐私边界（这段被主人反转过一次，别再改回去）
 *
 * 构造两样东西：
 *   · **聚合统计**：周期天数、有效天数、气压大变化日数与日期、打卡次数与标签分布、
 *     每日气压极值（让模型看清"哪几天气压在动"）
 *   · **逐条打卡明细** `check_in_detail`：时间戳、类别、标签、强度、气压、**备注原文**
 *
 * 我最初写的是"只发聚合统计，绝不包含逐条记录与备注原文"——**那个判断是错的**。
 * 标签只能归成六类，而"起床后右侧发紧"这种手写描述才包含可分析的细节；
 * 挡掉它等于让模型失去最有用的一路输入，报告就只剩复述数字。
 * 主人认为这次请求的收益值这个代价，于是口径反转，测试同步改成
 * `备注原文必须发出_它是最有分析价值的输入`。
 *
 * **仍然不发**：心率/腕温等体征的逐点读数（只发小时级统计）、任何设备或用户标识。
 *
 * 上传与否由用户在界面上显式决定——弹窗里逐条列出传什么、不传什么，
 * 本文件只负责"如果要传，传什么"。
 */
object AiDigest {
