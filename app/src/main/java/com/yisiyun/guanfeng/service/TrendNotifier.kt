 ...
 * ## 防打扰
 *
 * **等级一变就提醒**，冷却（[MIN_INTERVAL_MS]）只用来防止在两级之间抖动时刷屏。
 *
 * 这里原先写的是"只在升级到「高」时提醒一次"——**那是过期的说法**：
 * 从"高"回到"低"本身就是天气在动的信号（云散雨停），漏掉它等于只报坏消息不报好消息。
 * 主人要求"只要有变化就应该提醒"，实现也早已改成这样，是这段注释没跟上。
 * 宁可漏报一次，也不要变成一整天叮叮响的噪音源——被关掉通知的提醒等于不存在。
 */
